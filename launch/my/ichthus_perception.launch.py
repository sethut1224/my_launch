...
class Perception:
    def __init__(self, context):
        ...
        self.apollo_vlp16_param = os.path.join(
            get_package_share_directory('my_launch'), 'param/vlp-16.param.yaml'
        )
        self.apollo_hdl64_param = os.path.join(
            get_package_share_directory('my_launch'), 'param/hdl-64.param.yaml'
        )
        ...
        self.tensorrt_yolo_param_path = os.path.join(
            get_package_share_directory('my_launch'), 'param/tensorrt_yolo/',LaunchConfiguration('yolo_type').perform(self.context)+'.param.yaml'
        )
        ...
        self.lidar_centerpoint_model_param_path = os.path.join(
            get_package_share_directory('my_launch') , 'param', self.lidar_centerpoint_model_name+'.param.yaml'
        )
...
def generate_launch_description():
    ...
    vehicle_info_param_path_default = os.path.join(
        get_package_share_directory('my_launch'), 'param/vehicle_info.param.yaml'
    )

    lidar_centerpoint_param_path_default = os.path.join(
            get_package_share_directory('my_launch'), 'param/lidar_centerpoint.param.yaml'
    )

    lidar_apollo_instance_segmentation_param_path_default = os.path.join(
        get_package_share_directory('my_launch'), 'param/lidar_apollo_instance_segmentation.param.yaml'
    )

    tracker_setting_param_path_default = os.path.join(
        get_package_share_directory('my_launch'), 'param/default_tracker.param.yaml'
    )

    data_association_matrix_param_path_default = os.path.join(
        get_package_share_directory('my_launch'), 'param/data_association_matrix.param.yaml'
    )

    map_based_prediction_param_path_default = os.path.join(
        get_package_share_directory('my_launch'), 'param/map_based_prediction.param.yaml'
    )
